...
public class LayerActivation {
    ...
    public static Object derive(ActivationFunction activationFunction, Object input) {
        if(input instanceof Double input0d) {
            ...
            double[] output = new double[input1d.length];
            for(int i = 0; i < output.length; i++)
                output[i] = activationFunction.derivative(input1d[i]);
            ...
            for(int i = 0; i < output.length; i++)
                for(int j = 0; j < output[i].length; j++)
                    output[i][j] = activationFunction.derivative(input2d[i][j]);
            ...
            for(int i = 0; i < output.length; i++)
                for(int j = 0; j < output[i].length; j++)
                    for(int k = 0; k < output[i][j].length; k++)
                        output[i][j][k] = activationFunction.derivative(input3d[i][j][k]);
